...
| `internal/tui/*` | Bubble Tea v2 app: pages, overlays, wizard, components, theme, keys | `app.go` routes; `Deps` carries every outside-world seam (Doer, clock, clipboard, browser); pages are mostly pure render from precomputed data; `components` holds the chart primitives and the checklist shared by the wizard's two review steps and both in-app pickers |
